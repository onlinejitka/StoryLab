...
export default function App() {
  ...
  const handleForgeStory = async (e) => {
    ...
    const lengthLabels = { 
      ...
      medium: 'VELMI DLOUHÝ, POCTIVÝ PŘÍBĚH. Instrukce: Napiš minimálně 12 až 18 rozsáhlých odstavců...', 
      long: 'EPICKÝ ROZSÁHLÝ EPOS ROZDĚLENÝ NA KAPITOLY...' 
    };
    ...
    const finalTheme = theme.trim() ? theme : "Libovolné originální, milé a magické dobrodružství...";

    const userPrompt = `Parametry: ${finalHeroName}, ${ageLabels[ageGroup]}, ${tensionLabels[tension]}, ${finalTheme}`;
  ...
  return (
    <div className="min-h-screen bg-[#09070f] text-gray-100 font-sans antialiased">
      ...
      {/* 🌙 ŽIVÉ HLAVNÍ MENU (PŘIDÁNY OMALOVÁNKY) */}
      <nav className="w-full bg-[#0c0a16] border-b border-purple-950/40 px-4 py-4 md:px-8 shadow-lg">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-2 md:gap-3 text-xs font-bold">
            ...
            {/* NOVÁ ZÁLOŽKA OMALOVÁNKY PODLE PŘEDLOHY WEBU */}
            <a href="https://www.nocniknihovna.cz/omalovanky" className="flex items-center gap-2 px-3 py-2 rounded-xl bg-purple-950/20 border border-purple-900/40 text-purple-200 hover:text-white transition">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="w-4 h-4">
      ...
      <div className="p-4 md:p-8 pt-6">
        <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 order-2 lg:order-1 bg-[#120e24] border border-purple-950/40 rounded-2xl p-5 shadow-xl space-y-5 h-fit">
            ...
            <form onSubmit={handleForgeStory} className="space-y-5">
              ...
              <div className="bg-purple-950/20 border border-purple-900/40 p-3 rounded-xl space-y-2">
                ...
                <a href="https://buy.stripe.com/8x2fZh8CZ2H2eD73aQ9IQ0q" onClick={openStripePopup} className="text-[10px] text-emerald-400 hover:text-emerald-300 font-bold hover:underline pt-0.5 block text-center">Nemáte kód? Aktivovat Premium ➔</a>
              </div>
              ...
              <div>
                ...
                <input type="text" value={heroName} onChange={(e) => setHeroName(e.target.value)} placeholder="Např. Eliška, David..." className="w-full bg-[#191433] border border-purple-900/40 rounded-xl px-3 py-2.5 text-white text-sm" />
              </div>
          ...
          {/* PROSTŘEDNÍ PANEL (INTELIGENTNÍ PŘEPÍNÁNÍ PODLE STAVU KÓDU) */}
          <div className="lg:col-span-6 order-1 lg:order-2 bg-[#120e24]/30 border border-purple-950/20 rounded-2xl p-6 flex flex-col min-h-[550px] justify-center items-center relative">
            ...
            {error && (
              <div className="p-4 bg-red-950/40 border border-red-500/30 text-red-300 text-xs rounded-xl w-full text-center mb-4 flex flex-col items-center gap-2">
                <span>{error}</span>
                {error.includes("kód") && <a href="https://buy.stripe.com/8x2fZh8CZ2H2eD73aQ9IQ0q" onClick={openStripePopup} className="mt-1 bg-amber-500 hover:bg-amber-600 text-slate-950 px-4 py-2 rounded-xl font-bold text-xs">👉 AKTIVOVAT PREMIUM PŘÍSTUP ZA 75 KČ</a>}
              </div>
            )}
            ...
            {!isLoading && !story && (
              passcodeStatus.includes('✓') ? (
                /* STAV A: KÓD JE ZADÁN (Kopíruje design úspěchu z hlavního webu) */
                <div className="text-center p-6 max-w-md space-y-4 border border-emerald-500/30 bg-emerald-950/10 rounded-2xl shadow-xl animate-fadeIn">
                  ...
                  <p className="text-purple-200 text-sm leading-relaxed px-2">
                    Všechny funkce generátoru pohádek máte plně odemčené[cite: 2]. Nyní stačí v levé **Kovárně Příběhů** navolit parametry (jméno, věk, téma) a kliknout na tlačítko **Vykovat příběh ✨**.
                  </p>
                </div>
              ) : (
                /* STAV B: ANONYMNÍ UŽIVATEL (Zobrazí Paywall s přesným popisem služby) */
                <div className="text-center p-6 max-w-md space-y-5 border border-purple-950/60 bg-[#120e24]/50 rounded-2xl shadow-xl animate-fadeIn">
                  ...
                  <div className="pt-2">
                    <a href="https://buy.stripe.com/8x2fZh8CZ2H2eD73aQ9IQ0q" onClick={openStripePopup} className="inline-block bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 text-slate-950 font-black text-xs px-6 py-3 rounded-xl transition shadow-lg tracking-wider uppercase">AKTIVOVAT PŘÍSTUP ZA 75 KČ ➔</a>
                  </div>
                  <p className="text-[10px] text-purple-400/50 italic pt-2">Již máte svůj premium přístupový kód z e-mailu? Vložte ho vlevo nahoře a klikněte na Uložit kód[cite: 2].</p>
                </div>
              )
            )}
              ...
              <div className="w-full space-y-6 animate-fadeIn">
                ...
                {story.text && (
                  <div className="pt-4 border-t border-purple-950/60 space-y-3">
                    <button onClick={handlePlayAudio} className="w-full py-3 bg-[#191433] border border-emerald-500/30 rounded-xl font-bold text-emerald-400 hover:bg-emerald-500 hover:text-slate-950 text-sm">{isPlaying ? '🛑 Zastavit čtení' : '🔊 Přečíst příběh nahlas'}</button>
                  </div>
                )}
              </div>
            )}
                ...
                {loadingHistory ? <p className="text-[11px] text-purple-400/40 text-center py-4">Načítám příběhy...</p> : savedStories.length === 0 ? <p className="text-[11px] text-purple-400/40 text-center py-4 italic">V knihovně zatím nic není...</p> : (
                  <>
                    {freeStories.map((item) => (
                      <button key={item.id} type="button" onClick={() => handleSelectHistoryStory(item)} className={`w-full text-left p-2.5 rounded-xl border text-xs block truncate ${story?.id === item.id ? 'bg-emerald-950/20 border-emerald-500 text-emerald-300' : 'bg-[#191433] border-purple-920 text-purple-200'}`}>
